...
// Allow using useSidebar optionally.
...
const SidebarHeader = React.forwardRef<
  ...
  React.ComponentProps<"div">
>(({ className, ...props }, ref) => {
    const { state } = useSidebarOptional() ?? {};
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex h-16 items-center p-2", state === 'expanded' ? 'justify-start px-4' : 'justify-center', className)}
      {...props}
    />
  )
})
...
>(
  (
    ...
    const Comp = asChild ? Slot : "button"
    const { isMobile, state } = useSidebarOptional() ?? {};
